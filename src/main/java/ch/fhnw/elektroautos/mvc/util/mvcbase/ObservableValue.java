package ch.fhnw.elektroautos.mvc.util.mvcbase;
...
public final class ObservableValue<V> {
    ...
    public void setValue(V newValue) {
        if (value == newValue || (value != null
                && value.equals(newValue))) {  // no notification if value hasn't changed
            return;
        }
        V oldValue = value;
        value = newValue;
        ...
        listeners.forEach(listener -> {
            if (value == newValue || (value != null
                    && value.equals(newValue))) { // pre-ordered listeners might have changed this and thus the callback no longer applies
