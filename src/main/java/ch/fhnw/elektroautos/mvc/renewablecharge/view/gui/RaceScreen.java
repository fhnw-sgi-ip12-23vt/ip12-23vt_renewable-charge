...
public class RaceScreen extends GridPane implements ViewMixin<MainModel, ApplicationFXController> {
    ...
    public void setupModelToUiBindings(MainModel model) {
        onChangeOf(model.countdown).execute((oldVal, newVal) -> {
            if ((model.gameState.getValue() != GameState.RACE_COUNTDOWN)
                    && model.gameState.getValue() != GameState.RACE_RUNNING
            ) {
                return;
            }
            System.out.println("cow town: " + newVal);
