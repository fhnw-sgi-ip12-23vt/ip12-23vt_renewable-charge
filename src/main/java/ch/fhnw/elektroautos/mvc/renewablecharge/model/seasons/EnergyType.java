...
public class EnergyType {
     ...
     * @param imageName          the name to the image of the energy type
     * @throws IllegalArgumentException if the name is null or empty, or if the chancesOfEvent is out of range
     */
    public EnergyType(String translatePropName, float chancesOfEvent, int averagePackageSize, List<Event> events, String imageName) {
...
//        }
    ...
    /**
     * Generates a random energy package based on the configured weather events.
     *
    ...
    public EnergyPackage getWeatherSpecificPackage(WeatherConfiguration weatherConfiguration) {
        Event eventForWeather = events.stream().filter(e -> e.matchesWeather(weatherConfiguration)).findFirst()
                                      .orElseThrow();
        int   size            = (int) (this.averagePackageSize * eventForWeather.getMultiplier());
        return new EnergyPackage(size, this, eventForWeather, this.imageName);
    }
