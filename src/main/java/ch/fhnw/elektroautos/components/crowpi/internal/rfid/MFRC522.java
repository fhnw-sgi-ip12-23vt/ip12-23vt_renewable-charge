...
import ch.fhnw.elektroautos.components.crowpi.Component;
import ch.fhnw.elektroautos.components.crowpi.exceptions.RfidCollisionException;
import ch.fhnw.elektroautos.components.crowpi.exceptions.RfidException;
import ch.fhnw.elektroautos.components.crowpi.exceptions.RfidTimeoutException;
import ch.fhnw.elektroautos.components.crowpi.exceptions.RfidUnsupportedCardException;
...
public class MFRC522 extends Component {
        ...
        while (!uidComplete) {
            ...
            while (!selectComplete && iterationsLeft-- > 0) {
                ...
                if (knownLevelBits >= 32) {
                    ...
                    txFullBytes = (knownLevelBits / 8)
                            + 2; // Calculate number of fully known bytes (+2 for SEL and NVB)
    ...
    private byte[] calculateCrc(byte[] data) throws RfidTimeoutException {
        ...
        throw new RfidTimeoutException("CRC calculation deadline reached after "
                + PCD_CHECKSUM_TIMEOUT_MS
                + " milliseconds");
    }
